...
def model_predictions(data_dir, feature_fn, target_fn, shuffle=False,
                   batch_size=512, feature_norms=None, target_norms=None, noise=None, model=None, file_name=None):
    feature_file = h5py.File(data_dir + feature_fn, 'r')
    target_file = h5py.File(data_dir + target_fn, 'r')
    ...
    for i in range(n_batches):
        ...
        x = feature_file['features'][batch_idx:batch_idx + batch_size, :]
        y = target_file['targets'][batch_idx:batch_idx + batch_size, :]
        lat = target_file['lat'][batch_idx:batch_idx + batch_size]
        lon = target_file['lon'][batch_idx:batch_idx + batch_size]
        ...
        _model_predictions = model.predict(x)

        # unscale model predictions
        if target_norms is not None:
            _model_predictions = (_model_predictions/target_norms[1]) + target_norms[0]

        predictions.append(_model_predictions)
        true.append(y)
    np.savez(file_name + '.npz', predictions=predictions)
    targets_file = '/baldig/chemistry/EarthSystems/'+file_name.split('/')[1] + '/targets.npz'
    if not os.path.isfile(targets_file): np.savez(targets_file, true=true)
...
class DataGenerator(object):
    ...
    def return_generator(self, convo=False, tile=False, model=None, file_name=None):
    ...
    def model_predictions(self, model=None, file_name=None):
